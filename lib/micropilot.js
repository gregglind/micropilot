...
let Micropilot = exports.Micropilot = Class({
  ...
  upload:  function(url,options){
    ...
    this.data().then(function(data){
      myprefs.micropilotlog && console.log("micropilot-willupload:", JSON.stringify(data));
    ...
    if (pulseinterval){
      this.timerid = timers.setInterval(function(){
        ...
        if (Date.now() >= (that.start + that.duration)){
          console.log("pulse", resolve_this === that);
          that.resolve(resolve_this);
          console.log("I claim to have resolved");
          that.stop();
        }
        that.checking=false;
      },pulseinterval);
      ...
      if (duration <= timerunningsofar) { // really short intervals
        ...
        this.timerid = timers.setTimeout(function(){
          console.log("long");
          console.log("l",typeof(resolve_this),Object.keys(resolve_this),resolve_this == that);
          //this.resolve(resolve_this);
          that.resolve(1);
          console.log(resolve_this);
          that.stop();

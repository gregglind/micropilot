...
let moreindexeddb = {};
let canDoKeyRange = exports.canDoKeyRange = true;
try {
...
let microlog = exports.microlog = function () {
  if (myprefs.micropilotlog) console.log.apply(console, arguments);
};
...
let EventStore = exports.EventStore = Class({
  ...
  getAll: function () {
    ...
    this.db()
      .then(function (db) {
        let data = [];
        let req = db.transaction([that.collection], "readonly")
          .objectStore(that.collection)
          .openCursor();
        req.onsuccess = function (event) {
  ...
  /** promise to delete between keys
   *
   * Resolves onsuccess (`request.result`)
   *
   * @param int j min key
   * @param int k max key
   * @param bool openLeft
   * @param bool openRight
   *
   * @memberOf EventStore
   * @name deleteRange
   * @return promise
   */
  deleteRange: function (j, k, openLeft, openRight) {
    let that = this;
    let { promise, resolve } = defer();

    openLeft = openLeft === undefined ? false : openLeft;
    openRight = openRight === undefined ? false : openRight;

    let range = moreindexeddb.IDBKeyRange.bound(j, k, openLeft, openRight);
...
let Micropilot = exports.Micropilot = Class({
   ...
   * @param int k max key
   * @param bool openLeft
   * @param bool k openRight
   *
   * @memberOf EventStore
   * @name deleteRange
   * @return promise
   */
  deleteRange: function (j, k, openLeft, openRight) this.eventstore.deleteRange(j, k, openLeft, openRight),
  ...
  stop: function () {
    ...
    if (this.fuse !== undefined) this.fuse.stop();
    delete this.fuse;
    return this;
  },
  ...
  _watch: function (topic) {
    if (this._watched[topic]) return true;
